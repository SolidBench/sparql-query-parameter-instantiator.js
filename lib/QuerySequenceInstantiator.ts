...
import { IVariableTemplate, RawTerm } from './variable/IVariableTemplate';
...
export class QuerySequenceInstantiator {
  ...
  public async instantiateProviderSequence(n: number, user: string){
    const sequenceLength = this.logNormalRoundedUp(
      this.meanLogSequenceLength, this.stdLogSequenceLength
    );
    const sessionTransitionProbability = this.logNormal(
      this.meanLogTransitionProbability, this.stdLogTransitionProbability
    );
    ...
    const startQuery = currentSession.templates[currentSession.templates.length - 1];
    ...
    for (let i = 0; i < sequenceLength - 1; i++){
      ...
      let nextTemplateFilePaths = currentSession.templates[currentSession.templates.length-1].nextFilePaths;
      ...
      if (nextTemplates.length === 0){
        throw new Error("No valid next templates found for current template: " +
          currentSession.templates[currentSession.templates.length-1].name);
      }
  ...
  public startNewSession(
    ...
    let startQuery = this.sampleRandom(taskToTemplate[this.sampleRandom(queryTasks)]);
    const newSession = {
      sessionId: nSessions,
      templates: [startQuery],
      task: startQuery.task,
      sessionLength: this.logNormalRoundedUp(
        this.meanLogSessionLength, this.stdLogSessionLength
      ),
      ended: false
    }
    return newSession
  ...
  public addTemplateToSequence(
    ...
    return session.templates[session.templates.length - 1];
